<template>
  <view
    ...
    <view class="uni-forms-item__box">
      <view :class="['is-direction-' + labelPos]" class="uni-forms-item__inner">
        <view :style="{ width: labelWid , justifyContent: justifyContent }" class="uni-forms-item__label">
          <slot name="left">
            <uni-icons v-if="leftIcon" :color="iconColor" :type="leftIcon" class="label-icon" size="16"/>
            <text class="label-text">{{ label }}</text>
            <text v-if="required" class="is-required">*</text>
            <view v-if="label" class="label-seat"></view>
          </slot>
        </view>
        <view :class="{ 'is-input-error-border': msg }" class="uni-forms-item__content">
          <slot></slot>
        </view>
      </view>
      <view
